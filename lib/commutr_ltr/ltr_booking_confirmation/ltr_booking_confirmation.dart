...
class CommutrLtrBookingConfirmationScreen extends StatefulWidget {
  ...
  /// Login (pickup) shift time when both trips were Booking together.
  final String? loginTime;

  /// Logout (drop) shift time when both trips were Booking together.
  final String? logoutTime;

  const CommutrLtrBookingConfirmationScreen({
    super.key,
    this.isUpdate = false,
    this.successMessage,
    this.selectedDate,
  ...
  @override
  State<CommutrLtrBookingConfirmationScreen> createState() =>
      _CommutrLtrBookingConfirmationScreenState();
}

class _CommutrLtrBookingConfirmationScreenState
    extends State<CommutrLtrBookingConfirmationScreen>
  ...
  List<String> _allSelectedDates() {
    ...
    final end =
        (toRaw != null && toRaw.isNotEmpty) ? DateTime.parse(toRaw) : start;
  ...
  String _buildSubtitle() {
    ...
    // Both trips Booking together → show login and logout timings.
    if (rawLogin.isNotEmpty || rawLogout.isNotEmpty) {
      ...
      if (hasMultipleDates && datesPhrase.isNotEmpty) {
        buffer.write('Your ride has been Booking for $datesPhrase.\n');
      } else if (date != null && date.isNotEmpty) {
        buffer.write('Your ride has been Booking for $date.\n');
      } else {
        buffer.write('Your ride has been Booking.\n');
      }
    ...
    if (hasMultipleDates && datesPhrase.isNotEmpty && time.isNotEmpty) {
      return ' Booking successfully for $datesPhrase at $time.';
    }
    if (date != null && date.isNotEmpty && time.isNotEmpty) {
      return ' Booking successfully for $date at $time';
    }
  ...
  /// Clears the navigation stack and lands on the LTR home screen. Used by both
  /// the "Back to Home" button and the hardware/gesture back so the user always
  /// returns home, never to the booking flow behind this screen.
  void _goHome() {
    Navigator.pushAndRemoveUntil(
      context,
      MaterialPageRoute(
        builder: (context) => const CommutrLtrHome(),
      ),
      (route) => false,
    );
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      // Intercept the hardware/gesture back so it routes to home instead of
      // popping back into the booking flow.
      canPop: false,
